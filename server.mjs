...
async function callGemini(prompt, isImage = false, imageData = null) {
  ...
  for (let attempt = 1; attempt <= 5; attempt++) {
    try {
      ...
      if (r.ok && data.candidates?.[0]?.content?.parts?.[0]?.text) return data.candidates[0].content.parts[0].text;
      const errMsg = data.error?.message || "";
      if (r.status === 503 || errMsg.toLowerCase().includes("high demand") || errMsg.toLowerCase().includes("overloaded")) {
        if (attempt < 5) { await new Promise(res => setTimeout(res, 4000)); continue; }
      }

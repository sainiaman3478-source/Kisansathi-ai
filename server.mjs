...
app.get("/api/mandi", async (req, res) => {
  try {
    ...
    const limitValue =
      Math.min(
        Number(req.query.limit || 1000) || 1000,
        5000
      );
...
/* =========================================================
   FRONTEND
   IMPORTANT: API routes ऊपर हैं.
   अब React/Vite का dist serve होगा.
========================================================= */
  ...
  res.sendFile(
    ...
    err => {
      if (err) {
        ...
        res.status(500).send(
          "KisanSaathi frontend build नहीं मिला।"
        );
      }
    }
  );

...
app.get("/api/health",(req,res)=>res.json({ok:true,ai:Boolean(openai),mandi:Boolean(DATA_GOV_API_KEY)}));

app.post("/api/chat",async(req,res)=>{
  try{
    const message=String(req.body?.message||"").trim();
    if(!message)return res.status(400).json({error:"सवाल खाली है।"});
    if(!openai)return res.status(503).json({error:"OPENAI_API_KEY backend .env में सेट नहीं है।"});
  ...
  }catch(e){console.error(e);res.status(500).json({error:"AI service error. Backend logs देखें।"});}
});
...
app.get("/api/mandi",async(req,res)=>{
  try{
    if(!DATA_GOV_API_KEY)return res.status(503).json({error:"DATA_GOV_API_KEY backend .env में सेट नहीं है।"});
  ...
  }catch(e){console.error(e);res.status(500).json({error:"सरकारी मंडी डेटा नहीं मिल पाया। API key या data.gov.in connection जांचें।"});}
});
